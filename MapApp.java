...
public class MapApp {
	...
	InvalidFileException {
			...
			// check if header line in the file has < 3 columns
			if (info.length < 3)
				throw new InvalidFileException(graphFilepath);
			// parse the property names and create graph
			String[] edgeNames = new String[info.length - 2];
			for (int i = 2; i < info.length; ++i) {
				edgeNames[i - 2] = info[i];
			}
			NavigationGraph graph = new NavigationGraph(edgeNames);
			...
			while(scan.hasNextLine()) {
				...
				if (data.length != info.length) {
					throw new InvalidFileException("different number of properties than as described in the header");
				}
